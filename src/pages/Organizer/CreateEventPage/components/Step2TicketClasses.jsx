...
export const Step2TicketClasses = ({
  ...
  const [editingId, setEditingId] = useState(null);
  const [submitting, setSubmitting] = useState(false);
  const [deletingId, setDeletingId] = useState(null); // ID đang được xóa (dùng để hiện loading trên nút ×)
  ...
  const handleRemoveTicketClass = async (item) => {
    const { id, ticketClassId } = item;

    // Nếu đang sửa chính hạng vé này, hủy chỉnh sửa trước
    if (editingId === id) {
      handleCancelEdit();
    }

    // Hạng vé chưa lưu xuống DB (chỉ tồn tại trên UI) → xóa local thôi
    if (!ticketClassId) {
      setTicketClasses((prev) => prev.filter((tc) => tc.id !== id));
      return;
    }

    // Hạng vé đã có trong DB → gọi API xóa
    setDeletingId(id);
    setSubmitError("");

    try {
      await API.event.deleteTicketClass(eventId, ticketClassId);
      setTicketClasses((prev) => prev.filter((tc) => tc.id !== id));
    } catch (err) {
      setSubmitError(
        err?.response?.data?.message || "Có lỗi khi xóa hạng vé. Vui lòng thử lại."
      );
    } finally {
      setDeletingId(null);
    }
    ...
    <form className="create-event-step2" onSubmit={handleSubmit}>
                      ...
                      <td>
                        <div style={{ display: "flex", justifyContent: "center", gap: "8px" }}>
                          ...
                          <button
                            type="button"
                            className="create-event-icon-button"
                            onClick={() => handleRemoveTicketClass(item)}
                            title="Xóa"
                            disabled={deletingId === item.id}
                            style={{ opacity: deletingId === item.id ? 0.4 : 1, cursor: deletingId === item.id ? "not-allowed" : "pointer" }}
                          >
                            {deletingId === item.id ? "…" : "×"}
                          </button>
                        </div>
      ...
      <div className="create-event-actions">
        <button
          ...
          className="create-event-button create-event-button--ghost"
          onClick={onBack}
          disabled={submitting || editingId !== null || deletingId !== null}
        >
          Quay lại bước 1
        </button>
        <button type="submit" className="create-event-button" disabled={submitting || deletingId !== null}>
          {submitting ? "Đang lưu..." : "Tiếp tục"}
        </button>
      </div>
    </form>
